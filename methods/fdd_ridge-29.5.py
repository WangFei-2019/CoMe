...
def main_func(args, modelhander):
    logging.info("🚀 啟動 FDD-Ridge 剪枝算法 (包含細粒度分步探針評估)...")
    ...
    while modelhander.model.config.num_hidden_layers > args.target_layers:
        ...
        # 定義一個函數，掛載跳過 Hook 模擬物理刪除，然後測試 JSD
        def probe_current_state(stage_name):
            ...
            bypass_hook.remove()
            logging.info(f"   ➤ [{stage_name}] JSD: {jsd:.6f} | Cos: {cos:.4f}")

        # 🚨 [監控 1] 什麼都不做，直接跳過 L_p 測試 (純直接剪枝)
        logging.info("📈 階段效能診斷開始：")
        ...
        # 建議保持 0.8，也可以改成 1.0 用來跑您想看的退化現象
        damping_factor = 0.8 
        original_attn_w = L_c.self_attn.o_proj.weight.data.clone()
        original_ffn_w = L_c.mlp.down_proj.weight.data.clone()
        ...
        # 🚨 [監控 3] Attention 和 FFN 全部補償完成後測試
        probe_current_state("3. Attn + FFN 雙重補償")

        # 如果您發現在某個階段 JSD 變得極差，您可以通過代碼回滾！
        # 例如：如果 FFN 補償讓 JSD 翻倍了，你可以寫 L_c.mlp.down_proj.weight.data = original_ffn_w 來撤銷 FFN 的補償。
    ...
    save_path = getattr(args, 'save_path', f"fdd_ridge_pruned_{args.target_layers}L")
