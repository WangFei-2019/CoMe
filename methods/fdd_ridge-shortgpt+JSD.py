...
# 注意：这里不再需要 @torch.enable_grad()，全程在 inference 模式下运行，极大节省显存和时间！
@torch.inference_mode()
def main_func(args, modelhander):
    logging.info("🚀 啟動 FDD-ShortGPT-Ridge 算法 (基於 Block Influence 挑選 + 嶺迴歸補償)...")
    ...
    while modelhander.model.config.num_hidden_layers > args.target_layers:
        ...
        # =====================================================================
        # Phase 1: 纯粹的 ShortGPT BI (Block Influence) 敏感度扫描
        # =====================================================================
        bi_scores = {l: 0.0 for l in range(1, current_layer_num - 1)}
        total_tokens = 0
        
        modelhander.model.eval()
        # 由于不需要梯度，我们直接用正常的前向传播收集隐藏状态
        for i in tqdm(range(nsamples), desc=f"ShortGPT BI 探針掃描"):
            ...
            for l_idx in range(1, current_layer_num - 1):
                X_in = outputs.hidden_states[l_idx]
                X_out = outputs.hidden_states[l_idx+1]
                
                # 等价于 ShortGPT 的 block_influence: 1 - CosSim
                cos_sim = F.cosine_similarity(X_in.float(), X_out.float(), dim=-1).mean().item()
                # 累加 1 - CosSim (值越小，说明该层越"透明"，越该被剪)
                bi_scores[l_idx] += (1.0 - cos_sim)
                
            total_tokens += 1
            del outputs; torch.cuda.empty_cache()

        # 计算平均 BI 得分并挑选最低分（最不重要的层）
        scores = {l: (bi_scores[l]/total_tokens) for l in range(1, current_layer_num - 1)}
        prune_idx = min(scores, key=scores.get)
        comp_idx = prune_idx + 1
        logging.info(f"🎯 ShortGPT 鎖定層: Layer {prune_idx} (BI 破壞力最低分 {scores[prune_idx]:.6f})，將由 Layer {comp_idx} 吸收。")
        ...
        # 🚨 [監控 1] 原始直接剪除的威力
        probe_current_state("1. ShortGPT 原始直接剪除 (未補償)")

        # =====================================================================
        # Phase 2: 嶺迴歸補償 (保持 0.8 阻尼以防止 Variance Explosion)
        # =====================================================================
        hooks = SurgeryHooks()
        ...
        W_comp = torch.linalg.solve(ZtZ + (0.01 * torch.trace(ZtZ)/D_attn) * torch.eye(D_attn, device=compute_device), ZtY)
    ...
    logging.info(f"✅ FDD-ShortGPT-Ridge 剪枝結束。")
    save_path = getattr(args, 'save_path', f"fdd_shortgpt_ridge_{args.target_layers}L")
    modelhander.save(path=save_path)
    
    if hasattr(args, 'ppl_data') and args.ppl_data:
