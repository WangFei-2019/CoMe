...
def main_func(args, modelhander):
    ...
    while modelhander.model.config.num_hidden_layers > args.target_layers:
        ...
        # 方法二：基於 Golden Model 預測概率的加權（最精準，但需要 Logits）
        def collect_Z_and_solve(proj_layer, target_list, desc):
            ...
            for i in tqdm(range(num_calib), desc=desc):
                ...
                # ZtZ += Z.t() @ Z; ZtY += Z.t() @ Y
                weights = compute_token_difficulty_by_prob(Y) # 形狀 [Batch*SeqLen, 1]
        ...
        # 方法一：基於特徵模長 (L2 Norm) 的快速加權（不耗額外算力）
        def collect_Z_and_solve(proj_layer, target_list, desc):
            D_in, D_out = proj_layer.weight.shape[1], proj_layer.weight.shape[0]
            ZtZ = torch.zeros((D_in, D_in), device=compute_device)
            ZtY = torch.zeros((D_in, D_out), device=compute_device)
            ...
            for i in tqdm(range(num_calib), desc=desc):
                ...
                # ==========================================
                # 🌟 加權嶺迴歸核心邏輯：基於特徵模長計算權重
                # ==========================================
                # 1. 計算每個 Token 的特徵長度 (L2 Norm)
                token_norms = Y.norm(dim=-1, keepdim=True) # 形狀: [Batch*SeqLen, 1]
                
                # 2. 歸一化權重 (讓平均權重保持在 1.0 附近，避免破壞正則化項)
                # 使用平方或立方可以放大罕見詞的權重（這裡使用平方）
                weights = (token_norms / token_norms.mean()) ** 2 
                
                # 3. 限制最大權重，防止極端異常值讓矩陣崩潰 (Clip)
...
def compute_token_difficulty_by_prob(golden_logits_batch, input_ids_batch):
    """
    根據原模型的預測置信度來分配權重
    golden_logits_batch: [Batch, SeqLen, VocabSize]
    input_ids_batch: [Batch, SeqLen]
    """
    # 獲取 Golden Model 對真實 Token 的預測機率
    probs = F.softmax(golden_logits_batch.float(), dim=-1)
    
    # 提取對應 target token 的機率
    # 注意：第 i 個 logit 預測的是第 i+1 個 token
    # 為了對齊 SeqLen，我們在最後補一個 1.0 的 Dummy Prob
    ...
    for b in range(batch_size):
        ...
        target_probs[b, seq_len-1] = 1.0 # 最後一個 token 沒得預測，給默認值
        
    # 機率越低，權重越高（取倒數），並設定上下限
    # 假設機率 100% -> 權重 1.0
    # 假設機率 10% -> 權重 10.0 -> Clip 到 5.0
    weights = 1.0 / (target_probs + 1e-5) 
    weights = torch.clamp(weights, min=0.5, max=5.0)
    
    return weights.view(-1, 1) # 形狀: [Batch*SeqLen, 1]