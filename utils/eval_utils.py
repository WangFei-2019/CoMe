...
@torch.no_grad()
def eval_ppl(model, testenc, bs=1, device=None, desc=None):
    def _perplexity(nlls, n_samples, seqlen):
        return torch.exp(torch.stack(nlls).sum() / (n_samples * seqlen))
    
    if device is not None:
        orign_device = model.device
        model.to(device)
    else:
        orign_device = None
        device = model.device
    ...
    with tqdm(range(0,nsamples,bs), desc=f"{desc} Perplexity") as progress_bar:
        for i in progress_bar:
            ...
            curr_ppl = _perplexity(nlls, i + 1, model.seqlen)
            progress_bar.set_description(f"{desc} Perplexity {curr_ppl:.3f}")

    # Compute perplexity
    ppl = _perplexity(nlls=nlls, n_samples=nsamples, seqlen=model.seqlen)

    # Empty CUDA cache to save memory
    # torch.cuda.empty_cache()
    if orign_device is not None:
        model.to(orign_device)
...
torch.no_grad()
def eval_zero_shot(model_name, task_list=['piqa','winogrande','hellaswag','arc_challenge','arc_easy'], 
